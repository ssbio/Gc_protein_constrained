...
protein_cost_constraint = model.problem.Constraint(total_protein_cost_expression, lb=460, ub=460)
...
for glc_flux in glucose_flux_range:
    ...
    PGK_fluxes.append(solution.fluxes['PGK_b'])
        # Record the CS and ACONTb_f fluxes
...
# plt.suptitle('Φ = 0.46 g/gCDW', fontsize=16, fontweight='bold', verticalalignment='top')
...
plt.savefig("glucose_flux_variation_subplots_low.png")  # Specify your desired path and filename
...
with pd.ExcelWriter("results_with_fluxes_low.xlsx", engine='openpyxl') as writer:
